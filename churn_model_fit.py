...
import packages.churn_model_timestamps as cmt
import packages.churn_model_fitter as cmf
...
X_train_timestamped = cmt.include_timestamps(X_train)
X_val_timestamped = cmt.include_timestamps(X_val)

time.sleep(2)
print("model fit on X_train and X_val")
fit_model_X_train = cmf.model_fit(
    model = clf_LR,
    X = X_train, 
    y = y_train
    )
fit_model_X_val = cmf.model_fit(
    model = clf_LR,
    X = X_val, 
    y = y_val
    )
...
pred_X_train = cmf.predictions(
    fit_model = fit_model_X_train,
    X = X_train
    )
pred_X_val = cmf.predictions(
...
with open("./artefacts/churn-model-refactored.bin", "wb") as f_out:
    pickle.dump(fit_model_X, f_out) 
time.sleep(2)   
print("full model saved as churn-model-refactored.bin")
